...
def preprocess(dataset, file='tokenizer.json', prefix=''):
    tokenizer = loadTokenzier(dataset,file)
    dataset=map(lambda x:(tokenizer.encode(x['cs']),tokenizer.encode(x['en'])),dataset)
    dataset=filter(lambda x:1 < len(x[0].tokens)<100 and 1 < len(x[1].tokens)<100,dataset)
...
def main(args):
    ...
    dataset = load_dataset('wmt16', 'cs-en', split='train').to_dict()['translation']
    ...
    model = Transformer(args.vocab_dim, args.dim, args.atten_dim,recycle=6).to(device)
    print(f'args:{args}')
    ...
    for iter in range(args.epoch):
        ...
        for cs, en, cs_mask, en_mask in tqdm(train_data):
            ...
            cs_mask, en_mask = torch.tensor(cs_mask).to(device).detach(), torch.tensor(en_mask).to(device).detach()
    ...
    validation = load_dataset('wmt16', 'cs-en', split="validation")
    preprocess(validation, prefix='validation-')

    # test_data = MyDataSet(validation, tokenizer=tokenizer)
    # test_data = DataLoader(test_data, batch_size=256, collate_fn=collate_fn)
